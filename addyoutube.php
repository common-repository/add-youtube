...
- Wondering if there's a way to have addyoutube generate a thumbnail of the youtube video that shows up as one of the thumbnail options for the article when someone shares the blog post?
...
Only issue is I would LOVE to add the widget into another sidebar and I do not see this possibility. Can you have multiple Add YouTubes? 
...
- localization
- the images appear under the embedded Add YouTube videos. Is there any way to edit the z-index for Add YouTube CSS? I
- would like to use multiple instances of the Add YouTube plugin. I saw the reply that said to simply use multiple calls in one instance of the widget, but that doesn't let me choose different videos for different pages. 
...
if (isset($add_youtube)) return false;

require_once(dirname(__FILE__) . '/addyoutube.class.php');

$add_youtube = new AddYouTube();

add_action('wp_footer', array($add_youtube, 'wp_footer'));